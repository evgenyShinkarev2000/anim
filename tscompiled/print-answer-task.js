...
function stringHtmlToElement(htmlString) {
...
async function buildTask(dataPromise) {
    const data = await dataPromise;
    const tasks = data[0];
    const taskBlankString = data[1];
    const parentBlank = document.getElementsByTagName("form")[0].getElementsByClassName("answers")[0];
    let i = 0;
    tasks.forEach(task => {
        const divElement = stringHtmlToElement(taskBlankString);
...
async function postTask(answers) {
...
function paintAnswers(form, rightAnsers) {
...
initTask();
//# sourceMappingURL=print-answer-task.js.map